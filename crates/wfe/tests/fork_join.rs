...
use wfe_core::types::wfd_v22::{JoinRule, Wfd, WftTarget};
...
/// Koşum takımı (`ParStore` + mock'lar + fixture varyantları) `common`e taşındı:
/// `sim_valid_parity.rs` AYNI "gerçek akış" tarafını sürüyor, ikinci bir taklit
/// yazılmadı.
mod common;
use common::*;
...
async fn can_claim_many_matches_can_claim_row_by_row() {
    ...
    let targets: Vec<(Uuid, Option<String>)> = nodes
        .iter()
        .map(|n| (wfe_id, Some(n.to_string())))
        .collect();
...
async fn collapse_wins_race_and_losing_sibling_gets_collapsed_conflict() {
    ...
    assert!(!r.terminal);
    assert_eq!(
        r.current_node.as_ref().map(|n| n.id.as_str()),
...
async fn or_join_first_arrival_completes_and_cancels_siblings() {
    ...
    assert_eq!(
        cancelled.len(),
        2,
        "iki kardeş kol iptal marker'ı: {cancelled:?}"
    );
    assert!(
        !w.wfah
            .entries()
...
async fn cancelled_branch_cannot_act_after_quorum() {
    ...
    assert!(
        matches!(
            err,
            EngineError::InvalidInput(_) | EngineError::TransitionNotFound(_)
        ),
...
async fn expr_join_hr_alone_completes_and_cancels_siblings() {
    ...
    assert_eq!(
        w.join_rule,
        JoinRule::All,
        "paralel mod bitti, kural temizlendi"
    );
    assert!(w.branches.is_empty(), "E14/S2: kol satırları düştü");
    assert_eq!(
        marked_branches(&w, "_branch_cancelled"),
...
async fn fork_commit_carries_candidate_list_per_branch() {
    ...
    for node in &active {
        ...
        let (_, len) =
            entry.unwrap_or_else(|| panic!("kol '{node}' için c_a yazılmamış: {recorded:?}"));
...
async fn listable_only_viewer_can_never_claim_any_branch() {
    ...
    for node in nodes {
        ...
        assert!(
            !single,
            "tekil can_claim '{node}' için toplu karardan ayrıştı"
        );
    }
}
