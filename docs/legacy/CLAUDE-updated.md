...
```
{ "id", "name", "version", "description", "context", "start", "actions", "transitions", "listable", "terminal_when" }
```
...
- **`transitions`** — ordered array of rules. Human transitions: `id`, `when`, `action`, `c_a`, `wfes_effects`, `wft`. Autoexec transitions: `id`, `when`, `autoexec` (type + params + `on_error`), `wfes_effects`, `wft`.
- **`listable`** — defines who can see a WFE in listings without the right to act. Permanent or state-gated (with `when`). Uses `c_a` structure. Actors already in any `c_a` have listability implicitly. `x-visibility` on DynCtx fields still applies on top.
- **`terminal_when`** — predicate; when true the WFE is closed and no further actions are accepted.

## WFE Node Model (decided)

Every C_A is a **node**. A WFE flows between nodes. Two types:

**Human node** — has `c_a`; WFE waits for a matching Actor to perform the ACT.

**Autoexec node** — has `autoexec` instead of `c_a`; engine fires immediately on arrival. Used for REST calls, SQL queries, and calculations. Still writes to WFAH with system as actor.

Transition structure comparison:
```
Human:    { "id", "when", "action", "c_a",       "wfes_effects", "wft" }
Autoexec: { "id", "when",           "autoexec",  "wfes_effects", "wft" }
```

Three autoexec types:

```json
{ "autoexec": { "type": "rest", "method": "GET", "url": "...",
    "params":  { "tckid": { "ref": "$ctx.applicant.tckid" } },
    "result":  { "credit_score": "$.score", "credit_grade": "$.grade" },
    "on_error": { "wfes_effects": { "set": { "status": "fetch_failed" } }, "wft": { "c_a": [...] } }
} }

{ "autoexec": { "type": "sql",
    "query":   "SELECT limit FROM credit_limits WHERE grade = :grade",
    "params":  { "grade": { "ref": "$ctx.credit_grade" } },
    "result":  { "credit_limit": "$.limit" },
    "on_error": { "wfes_effects": { "set": { "status": "lookup_failed" } }, "wft": { "c_a": [...] } }
} }

{ "autoexec": { "type": "calc",
    "expressions": {
      "debt_ratio":   "ctx.amount_requested / ctx.applicant_income * 100",
      "within_limit": "ctx.amount_requested <= ctx.credit_limit"
    }
} }
```

Autoexec results referenced in `wfes_effects` via `$exec.result.field_name`.
Autoexec nodes can chain: `rest → sql → calc → human`. Each has `on_error` with its own `wft` to a recovery human node.

## TRIGGER and Conditional WFT (decided)

**TRIGGER** — a side-effect autoexec node that fires alongside an ACT. Does not change WFES. Always an autoexec node (`rest`, `sql`, `calc`) — nothing else. Two positions:
- **Action trigger** (root of transition) — fires every time this ACT fires, regardless of wft outcome
- **Branch trigger** (inside a `wft.conditions` entry) — fires only when that branch is taken

Full transition structure:
```
{ "id", "when", "action"|"autoexec", "c_a", "wfes_effects", "trigger"?, "wft" }
```
...
Conditional (ZEN expression per branch):
```json
"wft": {
  "conditions": [
    {
      "when": "$credit_info.amount_requested < 1000",
      "terminal": true,
      "wfe_end_response": {
        "status": "approved",
        "amount": { "ref": "$ctx.credit_info.amount_requested" }
      },
      "trigger": {
        "autoexec": { "type": "sql", "query": "INSERT INTO APPROVED_CREDITS ...", "params": {} }
      }
    },
    {
      "when": "$credit_info.amount_requested >= 1000",
      "c_a": [{ "c_orgu": "self", "c_r": [["self", "branchManager"]] }]
    }
  ]
}
```

**Complete example — dept manager approve with conditional routing:**
```json
{
  "id": "dept-manager-approve",
  "when": "$status == 'pending_review'",
  "action": "approve",
  "c_a": [{ "c_orgu": "self", "c_r": [["self", "creditDeptManager"]] }],
  "wfes_effects": {
    "set": { "status": "dept_approved", "dept_approved_by": "$actor", "dept_approved_at": "$timestamp" }
  },
  "trigger": {
    "autoexec": { "type": "rest", "method": "POST", "url": "https://audit.internal/log",
                  "params": { "wfe_id": "$wfe_id", "action": "dept_approved" } }
  },
  "wft": {
    "conditions": [
      { "when": "$credit_info.amount_requested < 1000",
        "terminal": true,
        "wfe_end_response": { "status": "approved", "amount": { "ref": "$ctx.credit_info.amount_requested" } },
        "trigger": { "autoexec": { "type": "sql", "query": "INSERT INTO APPROVED_CREDITS ...", "params": {} } }
      },
      { "when": "$credit_info.amount_requested >= 1000",
        "c_a": [{ "c_orgu": "self", "c_r": [["self", "branchManager"]] }]
      }
...
## Condition Evaluation — ZEN Engine (decided)

All `when` conditions (transition, `wft` branches, `listable`, trigger guards) are **ZEN expressions** — plain strings evaluated by [gorules/zen](https://github.com/gorules/zen).

WFES is exposed to ZEN as:
- `$<field>` — any DynCtx top-level field
- `$wfah` — full WFAH array: `[{ action, actor, at }, ...]`
...
ZEN replaces the previous JSON predicate format (`{ "field", "op", "value" }`) entirely.

**Effect/auto values** (`wfes_effects`, `start.auto`): `"$actor"`, `"$timestamp"`, `"$wfe_id"`, `"$action.input.field_name"`

**Dynamic references** (`c_orgu`, `wfes_effects`): `{ "ref": "$ctx.field_path" }`
...
**Term separation:**
- `x-visibility` / V — DynCtx field level: which fields an Actor can read
- `listable` / L — WFE level: which Actors can see the WFE exist; V still applies on top
