...
| `unknown_dollar_ref` | Bir DEĞER alanına yazılan `$`-string motorun tanıdığı biçimlerden biri olmalı: `$actor` · `$timestamp` · `$wfe_id` · `$node` · `$call.status` · `$call.wfe_id` · `$ctx.<yol>` · `$action.input.<yol>` · `$exec.result.<yol>` · `$call.result.<yol>` · `$env.ANAHTAR` (ara-değer dahil). Tanınmayan biçim (`$actor.role`, `$call.state`, `$ctxx.a`) HATA verir çünkü motor onu çözmez, alana o METNİ yazar — çalışma anında hata da log da yoktur. Denetlenen yerler çözücülerin bulunduğu yerlerdir: `wfes_effects.set`, `calls[].input`, `terminals[].wfe_end_response`, `autoexec[].config`; obje/dizi değerlerin İÇİ de gezilir (motor recursive çözer). `$` ile başlayan ama referans biçiminde OLMAYAN metin (`"$100 ödendi"`) sabittir, kurala takılmaz. ZEN ifadeleri bu kuralın DIŞINDADIR — namespace kümeleri ayrıdır. |
